...
public class ReadCsvFile extends Processable {
    ...
    public boolean run() {
        ...
        for (Integer key : this.columnsToMatch.keySet()) {
            String oldValue = this.columnsToMatch.get(key);
        ...
        for (Integer key : this.columnsToMatch.keySet()) {
            this.handler.logOutput("Column number: " + key + " to match value: " + this.columnsToMatch.get(key), false);
        }
        ...
        for (Integer columnNum : this.columnsToRead.keySet()) {
            String value = matchedLine.getValue(columnNum);
            String varName = this.columnsToRead.get(columnNum);
    ...
    protected class CsvFileLine {
        ...
        public boolean columnContains(int column, String value) {
            return this.values.get(column).equals(value);
        }

        public boolean columnsContain(HashMap<Integer, String> values) {
            for (Integer key : values.keySet()) {
                if (!columnContains(key, values.get(key))) {
                    return false;
                }
            }
            return true;
