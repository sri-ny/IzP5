...
public class ProcessPanelWorker implements Runnable {
        ...
        for (IXMLElement jobElement : spec.getChildrenNamed("job")) {
            ...
            if (!this.conditionChecker.checkOS(jobElement)) {
                ...
                for (IXMLElement actionElement : jobElement.getChildren()) {
                    ...
                    if (processable == null) {
                        logger.severe("Processable factory did not return a processable for action: " + actionElement.getName() + " for job: " + jobName);
                    } else {
    ...
    public void run() {
        ...
        for (ButtonConfig buttonConfig : this.buttonConfigs.get(Boolean.valueOf(this.result))) {
            String conditionid = buttonConfig.conditionid;
            ...
            unlockNext = buttonConfig.unlockNext;
            unlockPrev = buttonConfig.unlockPrev;
        }
    ...
    private static class ButtonConfig {
        ...
        final boolean unlockPrev;
        final boolean unlockNext;

        ButtonConfig(String conditionid, boolean unlockPrev, boolean unlockNext) {
